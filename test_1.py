...
class Paddle:
    def __init__(self, screen_width, screen_height):
        self.width = 100
        self.height = 15
        self.screen_width = screen_width  # Сохраняем ширину экрана для дальнейшего использования
        self.x = (screen_width - self.width) // 2
        self.y = screen_height - (self.height * 2)
        self.speed = 8

    def move_left(self):
        if self.x > 0:  # Ограничиваем движение влево
            self.x -= self.speed

    def move_right(self):
        if self.x + self.width < self.screen_width:  # Ограничиваем движение вправо
            self.x += self.speed
